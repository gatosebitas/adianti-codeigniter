	...
	*/
	public function onReload($param = NULL)
	{
		try
		{
			...
			$repository = new TRepository('ViewProyectosUnsa');
			$limit      = $param['limit'];
			// creates a criteria
			$criteria   = new TCriteria;
	...
	public function view($slug = FALSE)
	{
		if($slug != FALSE && is_scalar($slug)){
			...
			try
			{
				...
				// creates a repository for BlogCategoria
				$proyecto = new ViewProyectosUnsa($slug);
				$data['proyecto'] = $proyecto;
				$data['financiamiento'] = '';

				$repository      = new TRepository('ViewProyectoFinanciamiento');
				$criteria        = new TCriteria;
				$criteria->add(new TFilter('id_contrato', '=', $proyecto->id_proyecto));
	...
	private function actualizar_visitas($id_proyecto)
	{
		try
		{
			// open a transaction with database 'database'
			...
			$proyecto_contador = new ProyectosContador($id_proyecto);
		...
		catch(Exception $e) // in case of exception
		{
			...
			$proyecto_contador->id_proyecto = $id_proyecto;
	...
	public function resumen($slug = FALSE)
	{
		...
		if($slug != FALSE && is_scalar($slug)){
			try
			{
				...
				$json['data'][]['resumen'] = $proyecto->resumen_ejecutivo;
	...
	private function integrantes($id_proyecto)
	{
		try
		{
			...
			$sth  = $conn->prepare("SELECT
				bytsscom_bytsig.proyecto_equipo.id_proyecto,
				bytsscom_bytsig.proyecto_equipo.full_name_per,
				bytsscom_bytsig.proyecto_equipo.cargo_equipo,
				bytsscom_bytsig.proyecto_equipo.prof_equipo,
				bytsscom_bytsig.proyecto_equipo.estado_equipo,
				bytsscom_bytsig.proyecto_equipo.motivo_baja_equipo,
				bytsscom_bytsig.proyecto_equipo.id_corr_equipo
				FROM
				bytsscom_bytsig.proyecto_equipo
				WHERE
				((proyecto_equipo.estado_equipo)::text = 'A'::text) AND
				bytsscom_bytsig.proyecto_equipo.id_proyecto = ?
				ORDER BY
				bytsscom_bytsig.proyecto_equipo.id_proyecto ASC");

			$sth->execute(array($id_proyecto));
